...
bool cycle_helper(int winner, int loser, bool visited[]);
...
{
    // TODO
    qsort(pairs, pair_count, sizeof(pair), pair_comparator);
}
...
bool create_cycle(int winner, int loser)
{
    ...
    return cycle_helper(winner, loser, visited);
}
...
bool cycle_helper(int winner, int loser, bool visited[])
{
    ...
    for (int i = 0; i < candidate_count; i++)
    {
        if (locked[loser][i] && cycle_helper(winner, i, visited))
        {
            return true;
        }
    }
